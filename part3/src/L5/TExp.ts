...
export type AtomicTExp = NumTExp | BoolTExp | StrTExp | VoidTExp | UserDefinedNameTExp | AnyTExp | SymbolTExp ; // L51
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isEmptyTVar(x) ? makeOk(x.var) : isSymbolTExp(x) ? makeOk('symbol'):
